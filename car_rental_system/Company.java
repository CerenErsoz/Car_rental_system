...
public class Company {
    ...
    public boolean dateKontrol(Date start, Date end) {
        if(start.getYear()==end.getYear()&&start.getMonth()== end.getMonth() && end.getDay() - start.getDay() > 3) {
            return false;
        }
        else if(start.getYear()==end.getYear() && start.getMonth()<end.getMonth() && end.getMonth()-start.getMonth()==1) {
            if(end.getDay()+30-start.getDay()>4)
                return false;
        }
        else if(start.getYear()<end.getYear() && end.getYear()-start.getYear()==1 && start.getMonth()-end.getMonth()==11) {
            if(end.getDay()+30-start.getDay()>4)
                return false;
        }
        if(start.getDay() != currentDate.getDay() && start.getMonth() != currentDate.getMonth() && start.getYear() != currentDate.getYear()) {
            return false;
        }
        else if(start.getDay() != currentDate.getDay() && start.getMonth() == currentDate.getMonth() && start.getYear() == currentDate.getYear()) {
            return false;
        }
        return true;
    }
    ...
    public void listContract() {
        for(int i = 0; i < contract_count;i++) {
            if( contracts[i].isAvailable()==true)
                System.out.println("  "+contracts[i].getContract_id()+".Contract: Employee"+contracts[i].getEmployee_id() +" Customer" + contracts[i].getCustomer_id()+" Car" +contracts[i].getCar_id() + "   " +contracts[i].getStart_date().display() +"    " +contracts[i].getEnd_date().display());
        }
    }

    //her þeyi belli araç isteði
    public void addCarRequest(int office_id, int customer_id, String brand, String model, String clas, Date start_date, Date end_date) {
        CarRequest request = new CarRequest(office_id, customer_id, brand, model, clas, start_date, end_date, request_count+1);
        ...
        if(employee_id!=-1 && flag_date !=false && car_id!=-1) {
            Contract c = new Contract(employee_id, customer_id, car_id, start_date, end_date,contract_count+1,office_id,true);
    ...
    public void addCarRequestRandom(int office_id, String clas) {
        ...
        if(office_id == -1) {//-1 geldiðinde ragele ofis belirlenmesi için office count kadar bir aralýktan bir sayý belirlenir
            ...
            else System.out.println("Office not found!");
            if(employee_id != -1 && car_id != -1) {
                Contract c = new Contract(employee_id, customer_id, car_id, currentDate, end,contract_count+1,office_id,true);
    ...
    public void listCarRequest() {
        for(int i = 0; i < request_count; i++) {
            ...
            if(random_requests[i]!=null) {
                System.out.println("  "+random_requests[i].getCar_request_id()+".Car Request :  " + random_requests[i].getOffice_id() + " " + random_requests[i].getClas());
            }
            else {
                System.out.println("  "+requests[i].getCar_request_id()+ ".Car Request :  " + requests[i].getOffice_id() + " " + requests[i].getCustomer_id() + " "+ requests[i].getBrand()
                        + " " + requests[i].getModel() + " " + requests[i].getClas() + " " + requests[i].getStart_date().display() + " " + requests[i].getEnd_date().display());
            }
        }
    }
    ...
    public void listCustomer() {
        for(int i = 0; i< customer_count; i++) {
            System.out.println("  "+customers[i].getCustomer_id() + ".Customer " + customers[i].getName() + " " + customers[i].getSurname()+" "+customers[i].getCount());
        }
    }
    ...
    public  void listOffices() {
        for (int i = 0; i < office_count; i++) {
            if(offices[i].getOffice_id() != -1)//silinen officelerde hata almamak için kontrol
                System.out.println("  "+offices[i].getOffice_id() + ".Office " + offices[i].getPhone() + " " + offices[i].getAddress() + " " + offices[i].getCity());
        }
    }
    ...
    public void deleteOffice(int id_office){
        for(int i = 0; i < office_count;i++) {
            if(offices[i].getOffice_id() == id_office) {
                ...
                offices[i].setOffice_id(-1);//  eðer bir deðer olmaz null olursa döngüde hata veriyor (tamam :D)
        ...
        while(bool) {
                    ...
                    while ((satir = inputStream.readLine()) != null) {
                        ...
                        if (line[0].equalsIgnoreCase("addEmployee")) {
                        	...
                        	if(offices[Integer.parseInt(line[5])-1] == null) {//olmayan office çalýþan eklenemez
                    			...
                    			counter_employees2++;
                                offices[Integer.parseInt(line[5])-1].addEmployee(line[1],line[2],line[3],line[4],Integer.parseInt(line[5]),counter_employees2);
                    		}
                            ...
                            offices[Integer.parseInt(line[1])-1].deleteEmployee(Integer.parseInt(line[1]) ,Integer.parseInt(line[2]));
                           /* if(offices[Integer.parseInt(line[1])-1].officeControl(Integer.parseInt(line[1]))==false){
                                deleteOffice(Integer.parseInt(line[1]));
                        ...
                        if (line[0].equalsIgnoreCase("addCar")) {                       	            	
                        	if(offices[Integer.parseInt(line[5])-1] == null) {//olmayan office araba eklenemez 
                    			System.out.println("Office not found.");
                    			 ...
                    			 count_cars2++;
                                 offices[Integer.parseInt(line[5])-1].addCar(line[1],line[2],line[3],Integer.parseInt(line[4]),Integer.parseInt(line[5]),  count_cars2);
                    		}                        
                        }
                        ...
                        // REQUEST
                            ...
                            Date end = new Date(Integer.parseInt(arr1[0]), Integer.parseInt(arr1[1]), Integer.parseInt(arr1[2]));
                            addCarRequest(Integer.parseInt(line[1]), Integer.parseInt(line[2]), line[3], line[4], line[5],start ,end );
                        }
            ...
            //elle girilen kýsmýn else'i
                ...
                if(words[0].equalsIgnoreCase("addOffice")) {
                    ...
                    System.out.println("Enter employee id :");// ID belli olduðu için tek parametre yeterli.
                    i1 = sc.nextInt();
                    System.out.println("Enter office id :");
                    i2 = sc.nextInt();
           
                    offices[i2-1].deleteEmployee(i2,i1);
                   /*  if(offices[i2-1].officeControl(i2)==false){
                         deleteOffice(i2);
                       }*/
                }
                	...
                	if(office_count==0) {
                        ...
                        for(int i=0;i<office_count;i++) {
                            ...
                            System.out.println("Office"+offices[i].getOffice_id()+" profit : "+(offices[i].getIncome_sum()-offices[i].getExpenses_office())+" cp");
                            System.out.println("*******************************");
                        ...
                        if(count_nextday%3==0) {
                              ...
                              for(int a=0;a<office_count;a++) {
                              	if(offices[a].getIncome_sum()-offices[a].getExpenses_office()<0) {
                              		System.out.println("Office"+offices[a].getOffice_id()+": Buy a new car ("+offices[a].getClas()+")");
                              	}
                              	//Employee[] employees2 = offices[a].getEmployees1();
                              	/*if(offices[a].getEmployees()) {
                              		System.out.println("Close Office"+offices[a].getOffice_id());
                              	}*/
                              	//if(offices[a].getCount_e()==0)
                              		//System.out.println("Close Office"+offices[a].getOffice_id());
                              }
                        }
