...
public class Office {
    ...
    public Office(String phone, String address,String city, int office_id,
                  int capacity_car) {
    ...
    public int getIncome_sum() {return income_sum;}
    public void setIncome_sum(int income_sum) {this.income_sum = income_sum; }
	public String getPhone() {return phone;}
    public String getAddress() {return address;}
    public String getCity() {return city;}
    public int getOffice_id() {return office_id;}
    public int getCount_e() { return count_employee;}
    public void setCount_e(int count_employee) {this.count_employee = count_employee;}
    public void setPhone(String num) {phone = num;  }
    public void setAddress(String address_) {address = address_;  }
    public void setCity(String city_) { city = city_; }
    public void setOffice_id(int id) {office_id = id;  }
    public int getExpenses_office() { return expenses_office; }
    public void setExpenses_office(int expenses_office) {this.expenses_office = expenses_office;}


    public void addEmployee(String name, String surname, String gender, String birthdate, int off_id, int counter_employees2) {                           
        ...
        for (int i = 0; i < count_employee; i++)
        {
            if(employees[i] != null && employees[i].getName()!=("deleted")) {
                if(employees[i].getOff_id() == id_office)
                    System.out.println("  " + employees[i].getEmployee_id() + ".Employee " + employees[i].getName() + " " + employees[i].getSurname() + " " + employees[i].getGender() + " " + employees[i].getBirthdate() + " " + employees[i].getOff_id()+" "+ employees[i].EmployeeAvailable()+" "+employees[i].getRent_count_emp());
            }
        }
    }
        ...
        for(int i = 0; i < count_employee; i++) {
            if(employees[i]!=null) {
                if (employees[i].getEmployee_id() == id_employee && employees[i].getOff_id()==id_office) {
                    ...
                    employees[i].setName("deleted");//eðer null yaparsam döngüde hata veriyor (list)
                    compensation++;
                    count_e--;
                    System.out.println(count_e);
                }
        ...
        for (int i = 0; i < count_cars2 ; i++) {
            if(cars[i] != null)
            {
                if(cars[i].getOffice_id()==id_office){
                    System.out.println("  "+cars[i].getCar_id()+".Car "+cars[i].getBrand()+" "+cars[i].getModel()+" "+cars[i].getClas()+" "+cars[i].getKm()+" "+cars[i].getOffice_id()+" -"+cars[i].CarAvailable()+" "+cars[i].getRent_count());}
            }
    ...
    public void statistic(Contract [] c2, int count_c2, Customer [] c3, int count_c3,int office_id) {
        ...
        for(int j =0;j<count_c;j++) {
            if(cars[j]!=null) {
                if(max==cars[j].getRent_count()) {
                    System.out.print("Car"+cars[j].getCar_id()+";"+ cars[j].getBrand()+";"+cars[j].getModel()+"   ");
                }
            }
        }
        System.out.println();
        ...
        System.out.println();
        if(car!=null) {
        	 System.out.println("The car with the highest profit: Car"+car.getCar_id()+";"+car.getBrand()+";"+car.getModel());
        }
        ...
        for(int i=0;i<count_c2;i++) {
            if(c2[i].getOffice_id()==office_id) {
                date_fark= c2[i].getEnd_date().getDay()-c2[i].getStart_date().getDay()+1; 	//Þimdilik ay bazýnda
                sum_date+=date_fark;
                count_contract++;
            }
        }
            ...
            if(c2[l].getOffice_id()==office_id) {
                for(int m =0;m<count_c3;m++) {
                    if(c2[l].getCustomer_id()==c3[m].getCustomer_id()) {
                        if(c3[m].getCount()==max_customer) {
                            System.out.print("Customer"+c3[m].getCustomer_id()+";"+c3[m].getName()+";"+c3[m].getSurname()+"   ");
                        }
                    }
                }
            }
        ...
        for(int a =0;a<count_employee;a++) {
            if(employees[a]!=null) {
                if(employees[a].getRent_count_emp()==max_rent_employee) {
                    System.out.print("Employee"+employees[a].getEmployee_id()+";"+employees[a].getName()+";"+employees[a].getSurname()+"  ");
                }
            }
        }
        ...
        if(employees[temp]!=null) {
    		System.out.println("\nThe most profitable employee: Employee"+employees[temp].getEmployee_id()+";"+employees[temp].getName()+";"+employees[temp].getSurname()+"  "+maxxx+" cp.");
    	}
        summ/=count_e;
        ...
        for(int j=0;j<count_c;j++) {
            if(cars[j]!=null) {
                if(cars[j].getCarAvailabe()!=true) {
                    ...
                    System.out.println("  Car"+cars[j].getCar_id()+ " maintenance:" + "\t"+cars[j].getCar_maintenance2()+" + "+cars[j].getCar_maintenance()+" = "
                            +(cars[j].getCar_maintenance2()+cars[j].getCar_maintenance())+"\t("+cars[j].getKm()+"km)");
                }
                else {
...
//
   /* public int officeControl(int of_id){

        boolean flag=true;
        for (int i = 0; i <count_e ; i++) {
            if(employees[i]!=null ){
                if(employees[i].getOff_id()==of_id && employees[i].getName().equalsIgnoreCase("deleted") ){
                    count_e--;
                   // System.out.println(count_e);
               }
            }
    ...
    public int carKontrol(String b, String m, String c, int office_id, int count_cars2) {
        ...
        if(m.equalsIgnoreCase("*") && c.equalsIgnoreCase("*")) {//brand ý verilmiþ
            ...
            for(int i = 0; i < count_cars2; i++) {
                if(cars[i] != null) {
                    if(m.equalsIgnoreCase(cars[i].getModel()) && c.equalsIgnoreCase(cars[i].getClas()) && cars[i].getCarAvailabe() == true) {
                        id[count] = cars[i].getCar_id();
            ...
            for(int i = 0; i < count_cars2; i++) {
                if(cars[i] != null) {
                    if(m.equalsIgnoreCase(cars[i].getModel()) && b.equalsIgnoreCase(cars[i].getBrand()) && cars[i].getCarAvailabe() == true) {
                        id[count] = cars[i].getCar_id();
            ...
            for(int i = 0; i < count_cars2; i++) {
                if(cars[i] != null) {
                    if(c.equalsIgnoreCase(cars[i].getClas()) && b.equalsIgnoreCase(cars[i].getBrand()) && cars[i].getCarAvailabe() == true) {
                        id[count] = cars[i].getCar_id();
